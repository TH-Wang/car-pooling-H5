<template>
  <div>
    ...
    <!-- 顶部信息 -->
    <div class="header">
      <van-image :src="tour.img" class="image" width="1.15rem" height=".90rem" />
      <div class="header-detail">
        <div class="header-detail-title">{{tour.title}}</div>
        <div class="header-detail-aid">
          <p>出行时间：</p>
          <p>{{tour.startTime}} 至 {{tour.endTime}}</p>
        </div>
      </div>
    </div>
    ...
    <div class="page-title">
      ...
      <div class="info-field"><div>出行人数</div><span>{{order.userNum}}</span></div>
    </div>
    ...
    <div class="page-title" style="margin-bottom:.30rem">
      ...
      <div class="info-field"><div>付款时间</div><span>{{order.creationTime}}</span></div>
    </div>
...
export default {
  ...
  methods: {
    async handleReq () {
      ...
      const { carOrder, tour } = res.data.data
      this.tour = tour
      this.order = carOrder
    }
  },
